...
const LOCAL_DB_PATH = resolve(process.cwd(), ".data", "event-registration.json");
...
function shouldUseBlobStorage() {
  return Boolean(process.env.BLOB_READ_WRITE_TOKEN || process.env.NETLIFY || process.env.CONTEXT);
}

export function getStorageMode() {
  return shouldUseBlobStorage() ? "netlify-blobs" : "local-file";
}
...
export async function saveDatabase(database) {
  ...
  await writeFile(LOCAL_DB_PATH, JSON.stringify(normalized, null, 2), "utf8");
  return normalized;
}
...
export function validateSubmission(event, answers) {
